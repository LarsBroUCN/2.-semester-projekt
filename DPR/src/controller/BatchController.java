...
public class BatchController {
	...
	/**
	 * The method runs getPDList() to get a list of batches with notification status
	 * PENDING and DISCOUNT. Then it checks any of those notification should have
	 * status EXPIRED. Last but not least the method finds all batches with a
	 * notification that has status EXPIRED to return.
	 * <p>
	 * If a status is changed to EXPIRED, then the batch will be updated in the
	 * database.
	 *
	...
	/**
	 * The method first gets a list of batches with no notifications. Then if its
	 * reach the time where the batch should have a notification, it creates one.
	 * Last but not least the method finds all batches with a notification that has
	 * status PENDING to return.
	 * <p>
	 * If a new notification is created, then the batch will be updated in the
	 * database.
	 *
	...
	/**
	 * This method finds and combines two lists of batches, one list with
	 * notification status PENDING and the other with notification status DISCOUNT.
	 *
	 * @return returns a list of batches with notification status PENDING and
	 *         DISCOUNT
	 */
